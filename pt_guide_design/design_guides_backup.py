...
class Guide_Designer():
    ...
    def run(self):
        ...
        # guides_df = pd.concat([guides_df, nontargetting_df], axis=0)

        # nontargetting_df = self.design_nontargetting_guides(guides_df)

        safe_targetting_df = self.design_safe_targetting_guides()


        # print(f"Designed {total_guides} for {len(self.target_genes)} genes")
        # print(f"subpar design for {(len(subpar_genes))}")

        # with open("Subpar_guide_design_genes.txt", "w") as file:
        #     for gene in subpar_genes:
        #         file.write(f"{gene}\n")

        # guides_df.to_csv(f"{self.output_file}.csv", index=False, sep="\t", header=True)
        # summary_df.to_csv(f"{self.output_file}.summary.csv", index=False, sep="\t", header=True)


    def design_safe_targetting_guides(self):

        logging.info(f"Designing {self.args.safe_targeting_guides} safe targetting guides")

        results_df = pd.DataFrame()

        for safe_region in self.safe_regions:
            print(safe_region)
            self._design_safe_targetting_guides(safe_region)
    ...
    def _design_safe_targetting_guides(self, bed):
        ...
        self.annotate_guide_loc(guide_df["targetSeq"])
    ...
    def design_knockout_guides(self):
            ...
            with tqdm(total=total_tasks, desc="Designing Targetting Guides") as pbar:
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if len(guides) < self.num_guides:
                            subpar_genes.append(futures[future])
        ...
        print(f"Designed {total_guides} for {len(self.target_genes)} genes")
        print(f"subpar design for {(len(subpar_genes))} genes")
    ...
    def design_nontargetting_guides(self, df):
        ...
        logging.warning("CHECK Sequences are valid before contintuintg - remoge tttttt")
        # self.align_to_genome(df)

        # take designed guides
    ...
    def annotate_guide_loc(self, guides):

        # input single column do of guides (with pam) (Series)

        input_fasta = f"{self.outdir}/tmp/input.fasta"
        ...
        sam_df = sam_df.rename(columns={0:col_name, 1: "flag", 2: "chrom", 19: "XA"})
        print(sam_df)
...
def parse_args():
    ...
    parser.add_argument('--safe_targeting_guides',
                        help='number of guides to design for non-targetting group',
                        default=35,
                        type=int,
                        required=False)
