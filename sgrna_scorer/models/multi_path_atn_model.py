...
def create_model(sequence_input_shape, use_feature=False, num_filters=128, num_dense_neurons=128, dropout_rate=0.4):
    """
    ...
    x = layers.Embedding(input_dim=5, output_dim=64)(sequence_input)
    
    # Path 1: Small motifs
    path1 = layers.Conv1D(num_filters, kernel_size=3, activation='relu', 
                         kernel_initializer='he_normal', padding='same')(x)
    path1 = layers.Conv1D(num_filters//2, kernel_size=3, activation='relu',
                         kernel_initializer='he_normal', padding='same')(path1)
    path1 = layers.GlobalMaxPooling1D()(path1)
    path1 = layers.Dropout(dropout_rate)(path1)
    
    # Path 2: Medium motifs
    path2 = layers.Conv1D(num_filters, kernel_size=5, activation='relu', 
                         kernel_initializer='he_normal', padding='same')(x)
    path2 = layers.Conv1D(num_filters//2, kernel_size=5, activation='relu',
                         kernel_initializer='he_normal', padding='same')(path2)
    path2 = layers.GlobalMaxPooling1D()(path2)
    path2 = layers.Dropout(dropout_rate)(path2)
    
    # Path 3: Large motifs
    path3 = layers.Conv1D(num_filters, kernel_size=7, activation='relu', 
                         kernel_initializer='he_normal', padding='same')(x)
    path3 = layers.Conv1D(num_filters//2, kernel_size=7, activation='relu',
                         kernel_initializer='he_normal', padding='same')(path3)
    path3 = layers.GlobalMaxPooling1D()(path3)
    path3 = layers.Dropout(dropout_rate)(path3)
    
    # Concatenate CNN paths
    sequence_features = layers.Concatenate()([path1, path2, path3])
    
    if use_feature:
        # Position feature input
        feature_input = layers.Input(shape=(1,), name='feature_input')
        
        # Process feature separately
        feature_dense = layers.Dense(32, activation='relu')(feature_input)
        feature_dense = layers.Dense(16, activation='relu')(feature_dense)
        feature_dense = layers.Dropout(dropout_rate/2)(feature_dense)
        
        # Combine sequence features with processed position feature
        concatenated = layers.Concatenate()([sequence_features, feature_dense])
        inputs = [sequence_input, feature_input]
    else:
        concatenated = sequence_features
    ...
    # Create model
    if use_feature:
        model = models.Model(inputs=inputs, outputs=output, name='model_with_feature')
    else:
