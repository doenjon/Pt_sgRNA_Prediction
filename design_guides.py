...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("guide_design.log")  # Log to a file 
    ]
)
...
class Guide_Designer():
    ...
    def __init__(self, args):
        ...
        with open(args.saturating_genes, "r") as file:
            self.saturating_genes = file.read().strip().split("\n")
    ...
    def run(self):
        ...
        print(f"Designed {len(ko_guides_df)} for {len(self.target_genes)} genes")
        ko_guides_df.to_csv(f"{self.output_file_base}.KO.csv", index=False, sep="\t", header=True)
        ...
        nontargetting_df = self.design_nontargetting_guides(ko_guides_df)
        print(f"Designed {len(nontargetting_df)} non-targetting gudies")
        ...
        print(f"Designed {len(safe_targetting_df)} safe-targetting gudies")
        ...
        self.make_bed(safe_targetting_df).saveas(f"{self.output_file_base}.Safe.bed")


        sat_guides_df, sat_summary_df = self.design_saturating_guides()
        ko_guides = list(ko_guides_df["targetSeq"])

        # don't repeat guides already designed...
        sat_guides_df = sat_guides_df[~sat_guides_df["targetSeq"].isin(ko_guides)]

        sat_guides_df.to_csv(f"{self.output_file_base}.sat.csv", index=False, sep="\t", header=True)
        sat_summary_df.to_csv(f"{self.output_file_base}.sat.summary.csv", index=False, sep="\t", header=True)
        self.make_bed(sat_guides_df).saveas(f"{self.output_file_base}.sat.bed")
    ...
    def design_safe_targetting_guides(self):
        ...
        result_df = result_df.sort_values(by=["design_score"], ascending=False)
    ...
    def _design_safe_targetting_guides(self, bed):
        ...
        guide_df["offtarget_penalty"] = mismatch_seed_penalty*guide_df["mismatch_seed"] + mismatch_3_penalty*guide_df["mismatch_3"] + mismatch_4_penalty*guide_df["mismatch_4"]
        ...
        mismatch_all_filter  = ( (guide_df.mismatch_0 == 0) & (guide_df.mismatch_1 == 0) & (guide_df.mismatch_2 == 0))        
        graf_filter          = ( guide_df.GrafEtAlStatus == "GrafOK" )
        variant_filter       = ( guide_df.has_variant == False )
        blast_filter         = ( guide_df.start.notna() )

        filters = mismatch_0_filter & mismatch_1_filter & mismatch_2_filter & graf_filter & variant_filter & blast_filter
        ...
        filter_summary["average_mismatch_seed"] = guide_df["mismatch_seed"].mean()
        filter_summary["average_mismatch_3"] = guide_df["mismatch_3"].mean()
        filter_summary["average_mismatch_4"] = guide_df["mismatch_4"].mean()

        column_order = ["gene", "guides_designed", "average_CRISPROn", "average_design_score", "average_mismatch_seed", \
                        "average_mismatch_3", "average_mismatch_4", "PP:mismatch_0", "PP:mismatch_1", \
                        "PP:all_mismatch", "PP:graf", "PP:no_variant", "PP:blast_filter", "PP:all", "guides_pre_filter" , "guides_post_filter"]
        ...
        guide_df = self.find_overlapping(guide_df, overlap_threshold=0.6)

        outfile = os.path.join(self.args.o, "work", bed_name, f"{bed_name}.filterd.withoverlapping.csv")
        guide_df.to_csv(outfile, header=True, index=False, sep="\t")

        guide_df = guide_df[guide_df["overlapping"] == False]
        after_overlap_filter = len(guide_df)

        filter_summary["guides_post_filter"]  = len(guide_df)

        bed_out = os.path.join(outdir, "final_guides.bed")
    ...
    def _design_knockout_guides(self, gene):
        ...
        summary_data["average_mismatch_seed"] = guide_df["mismatch_seed"].mean()
        summary_data["average_mismatch_2"] = guide_df["mismatch_2"].mean()
        summary_data["average_mismatch_3"] = guide_df["mismatch_3"].mean()
        summary_data["average_mismatch_4"] = guide_df["mismatch_4"].mean()

        column_order = ["gene", "guides_designed", "average_position", "max_position", "average_CRISPROn", "average_design_score", "average_mismatch_seed", \
                        "average_mismatch_2", "average_mismatch_3", "average_mismatch_4", "PP:pos<0.9", "PP:mismatch_0", "PP:mismatch_1", \
                        "PP:all_mismatch", "PP:graf", "PP:no_variant", "PP:in_valid_exon", "PP:blast_filter", "PP:all", "guides_pre_filter" , "guides_post_filter"]
        ...
        bedfile = os.path.join(self.args.o, "work", gene, f"{gene}.filtered.bed")
        self.make_bed(guide_df).saveas(bedfile)
    ...
    def design_saturating_guides(self):

        logging.info(f"Designing all guides/gene for {len(self.target_genes)} genes")
        ...
        with ThreadPoolExecutor(max_workers=self.args.t) as executor:
            ...
            futures = {executor.submit(self._design_saturating_guides, target_gene): target_gene for target_gene in self.saturating_genes}
    ...
    def _design_saturating_guides(self, gene):
        ''' gene - String name of gene '''

        os.makedirs(os.path.join(self.args.o, "work"), exist_ok=True)
        

        logging.info(f"Designing guides for {gene}_saturating")

        outdir = os.path.join(self.args.o, "work", f"{gene}_saturating")
        os.makedirs(outdir, exist_ok=True)
        ...
        guide_df, filter_summary = self.filter_saturating_guides(gene, guide_df, gff)
    ...
    def filter_saturating_guides(self, gene, df, gff):
        # df of guides
        ...
        mismatch_3_penalty = 0.05
        mismatch_4_penalty = 0.025
        df["offtarget_penalty"] = mismatch_seed_penalty*df["mismatch_seed"] + mismatch_2_penalty*df["mismatch_2"] + mismatch_3_penalty*df["mismatch_3"] + mismatch_4_penalty*df["mismatch_4"]
        ...
        mismatch_seed_filter20 = ( df.mismatch_seed <= 20 )
        mismatch_2_filter20    = ( df.mismatch_2 <= 20 )
        mismatch_all_filter    = ( (df.mismatch_0 == 0) & (df.mismatch_1 == 0) & (df.mismatch_seed <= 20) & (df.mismatch_2 <= 20))        
        ...
        df_filter = pd.concat([df, mismatch_0_filter, mismatch_1_filter, mismatch_seed_filter20, mismatch_2_filter20, graf_filter, variant_filter, exon_filter, blast_filter], axis=1)
        outfile = os.path.join(self.args.o, "work", gene, f"{gene}.all.score.plusfilter.csv")
        df_filter.to_csv(outfile, header=True, index=False, sep="\t")


        # Only part changed
        filters =  mismatch_0_filter & mismatch_1_filter & exon_filter & blast_filter

        filter_summary = {}
        
        # filter_summary["PP:pos<0.9"]          = pos_filter.sum()
        filter_summary["PP:mismatch_0"]       = mismatch_0_filter.sum()
        filter_summary["PP:mismatch_1"]       = mismatch_1_filter.sum()
        # filter_summary["PP:mismatch_seed<20"] = mismatch_seed_filter20.sum()
        # filter_summary["PP:mismatch_2<20"]    = mismatch_2_filter20.sum()
        # filter_summary["PP:all_mismatch"]     = mismatch_all_filter.sum()
        # filter_summary["PP:graf"]             = graf_filter.sum()
        # filter_summary["PP:no_variant"]       = variant_filter.sum()
    ...
    def filter_KO_guides(self, gene, df, gff):
        ...
        df_filter = pd.concat([df, pos_filter, mismatch_0_filter, mismatch_1_filter, mismatch_seed_filter20, mismatch_2_filter20, graf_filter, variant_filter, exon_filter, blast_filter], axis=1)
        outfile = os.path.join(self.args.o, "work", gene, f"{gene}.all.score.plusfilter.csv")
        df_filter.to_csv(outfile, header=True, index=False, sep="\t")

        filters = pos_filter & mismatch_0_filter & mismatch_1_filter & mismatch_seed_filter20 & mismatch_2_filter20 & graf_filter & variant_filter & exon_filter & blast_filter
        ...
        df = df.sort_values(by="design_score", ascending=False).copy()

        df = self.find_overlapping(df)

        outfile = os.path.join(self.args.o, "work", gene, f"{gene}.filterd.withoverlapping.csv")
        df.to_csv(outfile, header=True, index=False, sep="\t")

        before_overlap_filter = len(df)
        no_dedup_df = df.copy(deep=True)
        df = df[df["overlapping"] == False]
        after_overlap_filter = len(df)

        if before_overlap_filter >= self.num_guides and after_overlap_filter <= self.num_guides:
            
            overlap=0.2
            while len(df) < self.num_guides:
                overlap += 0.1
                df = no_dedup_df
                df = self.find_overlapping(df, overlap_threshold=overlap)
                df = df[df["overlapping"] == False]
            ...
            logging.warning(f"{gene} -- overlap filtering removed too many guides - Changed overlap threshold to {overlap}")
    ...
    def find_overlapping(self, df, overlap_threshold=0.2):
        # only for KO guides - remove guides that are overlapping
        # assume df is sorted correctly (be desire of guide)
        # overlap is threshold overlapping to throw out

        df["overlapping"] = False
        df = df.reset_index(drop=True)

        for guide1, _ in df.iterrows():
            # print(f"{guide1} - {df['targetSeq'][guide1]}")
            for guide2 in range(0, guide1):

                # print(f"{guide1} - {df['targetSeq'][guide1]}, {guide2} - {df['targetSeq'][guide2]}")

                if df["overlapping"][guide2] == False:
                    stop = min(df["stop"][guide1], df["stop"][guide2])
                    start = max(df["start"][guide1], df["start"][guide2])

                    # BAD BAD -- assuming guide length of 23
                    overlap = (stop - start) / 23

                    if overlap > 0 and overlap >= overlap_threshold:
                        # print(f"found overlap of {overlap}")
    ...
    def count_offtargets(self, offtarget_df):
        ...
        pivot_table["mismatch_seed"] = pivot_table["seed_mismatch_0"]        
        offtarget_df = offtarget_df.merge(pivot_table, left_on='guideSeq', right_index=True, how='left')
        ...
        offtarget_df = offtarget_df[["guideSeq", "mismatch_seed", "mismatch_0", "mismatch_1", "mismatch_2", "mismatch_3", "mismatch_4"]]
    ...
    def region_in_exon(self, gene, chrom, start, stop, strand, gff):
        ...
        # Actually -- also checks in CDS - not just exon
        ...
        if len(genes) != 1: # guide must be in exactly one gene
            # print(f"WARNING - {gene} - region_in_exon() - guide at cutsite {cutsite}: found more than 1 gene")
            return False 
    
        curr_gene = genes[0]["ID"][0].split(":")[1] # gene must be targeted gene
        if curr_gene != gene: 
            # print(f"WARNING - {gene} - region_in_exon() - guide at cutsite {cutsite}: found wrong gene")
            return False


        CDSs = [c for c in gff.region(seqid=chrom, start=min(start, stop), end=max(start, stop), completely_within=False, featuretype="CDS")]

        if len(CDSs) == 0:
            # print(f"WARNING - {gene} - region_in_exon() - guide at cutsite {cutsite}: not in CDS")
            return False
        ...
        if len(exons) == 0: # guide must be in exactly one exon
            # print(f"WARNING - {gene} - region_in_exon() - guide at cutsite {cutsite}: found no exons")
            return False 
        
        if len(exons) != 1: # guide must be in exactly one exon
            # print(f"WARNING - {gene} - region_in_exon() - guide at cutsite {cutsite}: found more than 1 exon")
            # print(f"\t{exons}")
            return False 
        ...
        if (cutsite - cutsite_buffer >= curr_exon.start) and (cutsite + cutsite_buffer <= curr_exon.stop):
            ...
            for exon in gff.children(gene, featuretype='exon', order_by="seqid"):                
                ...
                if exon.start <= cutsite <= exon.stop:
                    exon_num = int(exon.attributes["rank"][0])
                    return exon_num
            ...
            #     inc = 1
            # elif gene.strand == "-":
            #     exon_ct = len(list(gff.children(gene, featuretype='exon', order_by="seqid"))) + 1
            #     inc = -1
            ...
            #     # cutsite is in this exon 
            #     if exon.start <= cutsite <= exon.stop:
            #         # print(f"WARNING - {gene} - region_in_exon() - found exon count {exon_ct}")

            #         return exon_ct
            #         # return exon_ct
            # # print(f"WARNING - {gene} - region_in_exon() - no problems but can't find correct exon number")
            # # print("########################################")
    ...
    def find_guide_matches(self, df):
        # Make sure a non-targetting guide does not target the genome
        # not named well...
...
def parse_args():
    ...
    parser.add_argument('--saturating_genes',
                        help='File of genes to generate guides for',
                        type=str,
                        required=True)
    ...
    parser.add_argument('--gff',
                        ...
                        default="resources/Phaeodactylum_tricornutum.ASM15095v2.57.gff3",
                        type=str,
                        required=False)
    
    parser.add_argument('--bed',
                        help='bed for genome to use for guide design',
                        default="resources/Phaeodactylum_tricornutum.ASM15095v2.57.bed",
                        type=str,
                        required=False)
